"""Fetch public-source confirmation details for all PSDQ inspection rows.

This pass follows the targeted public-map inspection packet. It reads the
40-row inspection CSV, opens the public DGHS profile URL and public OSM API
record for each prioritized candidate feature, records what was retrieved, and
keeps every row open unless public evidence is strong enough for a later
manual source decision.
...
OUT_CONFIRMATION_CSV = OUT_DIR / "psdq-bgd-facility-validation-public-source-confirmation-targeted-rows.csv"
OUT_CONFIRMATION_SUMMARY_JSON = (
    OUT_DIR / "psdq-bgd-facility-validation-public-source-confirmation-targeted-rows-summary.json"
)

METHOD = "ai_public_source_confirmation_targeted_rows_v1"
STATUS = "ai_public_source_confirmation_targeted_rows_not_human_validation"
USER_AGENT = "ADB-AI-Research-PSDQ-targeted-row-public-source-confirmation/1.0 (public evidence audit)"
NON_CLAIM = (
    "This is an AI-first public-source confirmation packet for targeted PSDQ "
    "public-map inspection rows. It retrieves public DGHS profile pages and "
    "public OSM API feature records. It is not human validation, not ground "
    "truth, not a facility-quality assessment, and not a service-access "
    "estimate."
)
...
def profile_signal(row: dict[str, Any], profile_body: str) -> dict[str, Any]:
    ...
    dghs_id = str(row.get("dghs_id") or "")
    if not dghs_id:
        profile_url = str(row.get("dghs_public_profile_url") or "")
        parts = profile_url.rstrip("/").split("/")
        dghs_id = parts[-2] if len(parts) >= 2 and parts[-1] == "profile" else ""
    return {
...
def upazila_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key, items in grouped.items():
        ...
        lane_counter = Counter(row["public_source_confirmation_lane"] for row in items)
        output.append(
            {
                ...
                "dghs_profiles_retrieved": sum(1 for row in items if row["dghs_profile_retrieved"]),
                "osm_candidate_api_records_retrieved": sum(1 for row in items if row["candidate_osm_api_retrieved"]),
                "rows_with_candidate_name_score_at_least_0_75": sum(
                    1 for row in items if to_float(row["candidate_name_score_from_live_tags"]) >= 0.75
                ),
                "dominant_confirmation_lane": lane_counter.most_common(1)[0][0],
                "source_repair_rows": lane_counter.get("source_repair_public_sources_retrieved_keep_open", 0),
                "possible_same_facility_rows": lane_counter.get(
                    "possible_same_facility_candidate_needs_manual_location_check", 0
                ),
                "name_conflict_rows": lane_counter.get("candidate_feature_retrieved_but_name_conflict_keep_open", 0),
                "zero_osm_context_rows": lane_counter.get(
                    "zero_osm_context_candidate_outside_upazila_keep_open", 0
                ),
            }
        )
    return sorted(output, key=lambda row: (-row["priority_1_rows"], -row["rows"], row["district_name"], row["upazila_name"]))


def main() -> None:
    if not IN_INSPECTION_CSV.exists():
        raise FileNotFoundError(IN_INSPECTION_CSV)

    source_rows = read_csv(IN_INSPECTION_CSV)
    ...
    for index, row in enumerate(source_rows, start=1):
        ...
        output_rows.append(
            {
                "confirmation_id": f"PSDQ-BGD-PST-{index:03d}",
    ...
    scope = {
        ...
        "priority_1_rows_checked": sum(1 for row in output_rows if row["priority_scope"] == "priority_1_high_exposure"),
        ...
        "rows_with_dghs_profile_token_support": sum(
            ...
            if to_float(row["dghs_profile_facility_token_coverage"]) >= 0.5
            or to_float(row["dghs_profile_public_name_token_coverage"]) >= 0.5
        ),
        "rows_with_candidate_name_score_at_least_0_75": sum(
            1 for row in output_rows if to_float(row["candidate_name_score_from_live_tags"]) >= 0.75
        ),
    ...
    summary = {
        ...
        "priority_scope_counts": count_rows(output_rows, "priority_scope"),
        "upazila_confirmation_rows": upazila_rows(output_rows),
        "row_card_rows": [
            {
        ...
        "confirmation_notes": [
            "This packet records public-source retrieval and tag support for all targeted inspection rows.",
    ...
    print(
        "Built BGD targeted-row public-source confirmation: "
        f"{len(output_rows)} rows, "
        f"{scope['dghs_profiles_retrieved']} DGHS profiles retrieved, "
        f"{scope['osm_candidate_api_records_retrieved']} OSM API records retrieved, "
        "0 closed."
    )
