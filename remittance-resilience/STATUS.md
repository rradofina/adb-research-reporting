...
- **2026-06-17 (flow-weighting L3 formalization + packet rebuild):**
  ...
  rule now requires at least 90 percent latest-period RPW corridor matching
  to public KNOMAD bilateral-flow estimates, retracts/reframes the
  equal-weighted baseline if more than one top-five entry changes, and
  prohibits household-transaction, informal-channel, country-performance, or
  maturity-promotion language. `scripts/sprint-flow-weighted-cost.py` now
  emits `goal_level: L3 sensitivity module`, the repaired program baseline
  order, the matched-corridor quote order, the flow-weighted order, and the
  matched-corridor share. Regenerated artifacts show 140 of 142 RPW corridors
  matched (0.9859), repaired baseline `KGZ`, `WSM`, `TON`, `NPL`, `VUT`,
  matched-corridor equal-weighted order `KGZ`, `WSM`, `TON`, `VUT`, `NPL`,
  and flow-weighted order `KGZ`, `NPL`, `VUT`, `WSM`, `TON`. Updated
  `results.md`, `sensitivity.md`, `coverage.md`, `limitations.md`, the
  working paper, review addenda, showcase registry, and the remittance
  showcase route. Rebuilt `review-packets/remittance-resilience-2026-06-18/`
  and `review-packets/remittance-resilience-2026-06-18.zip`. Closeout
  verification completed 2026-06-19: `npm run build`, six gates
  (`check-citations`, `check-composite-headline`, `check-wip`,
  `check-dmc-framing`, `check-banned-words`, `check-versions`), targeted
  contradiction search, and Chrome/CDP browser QA on
  `/showcase/remittance-flow-weighting` at 1365px and 375px. Screenshots:
  `reporting-site/qa/showcase-remittance-l3-desktop.png` and
  `reporting-site/qa/showcase-remittance-l3-mobile.png`. This is an L3
  repair re-close, not a maturity promotion or human-final claim.
...
- None at AI-first depth for the parser repair, median-cost update, and public
  KNOMAD flow-weighting L3 module. Owner-gated upgrades for §18.5 human-final:
  ...
  - Central-bank, transaction-volume, informal-channel, or household microdata
    validation where those sources require owner identity, permission, or
    external reviewer participation.
