...
function buildSchoolHeatModel(report: ShowcaseReport, data: JsonValue): AuditModel {
  ...
  const correlations = safeRows(data.correlations);
  const correlation = (label: string) =>
    correlations.find((row) => String(row.label) === label) || {};
  const indexCorrelation = correlation("Old index vs heatwave affected count");
  const populationCorrelation = correlation("Child population vs heatwave affected count");
  const signed = (value: unknown) => {
    const parsed = numberValue(value);
    return `${parsed >= 0 ? "+" : ""}${formatNumber(parsed, 2)}`;
  };
  const laneRows = safeRows(data.sensitivity_runs).map((row) => {
  ...
  const sourceTrail = sourceFacts(report, data);
  if (data.sources?.unicef?.url) {
    sourceTrail.push({ label: "UNICEF annex PDF", value: String(data.sources.unicef.url) });
  }
  if (data.sources?.unicef?.sha256) {
    sourceTrail.push({ label: "UNICEF PDF SHA-256", value: String(data.sources.unicef.sha256) });
  }
  if (data.sources?.wdi_enrollment?.api_url) {
    sourceTrail.push({ label: "WDI enrollment API", value: String(data.sources.wdi_enrollment.api_url) });
  }

  const componentCards = [
    {
      key: "observed-rank",
      value: `${formatNumber(summary.cambodia_heatwave_affected_rank)}/${formatNumber(summary.heatwave_major_rows_n)}`,
      label: "Cambodia affected-count rank",
      note: "Cambodia led the inherited proxy but has the smallest affected-student count in the heatwave-major subset.",
      status: "flag",
    },
    {
      key: "proxy-correlation",
      value: signed(indexCorrelation.spearman),
      label: "proxy vs affected count",
      note: "The deterministic 95% bootstrap interval spans -1.00 to +1.00 across six selected observations.",
      status: "dropped",
    },
    {
      key: "population-correlation",
      value: signed(populationCorrelation.spearman),
      label: "child population vs count",
      note: "Demographic scale carries much more of the observed count order; this is descriptive, not causal.",
      status: "survived",
    },
    {
      key: "school-day-join",
      value: formatNumber(summary.school_day_exposure_outcome_rows_n),
      label: "school-day outcome rows",
      note: "No row yet aligns instructional days, local heat, enrolled students, and an observed educational outcome.",
      status: "dropped",
    },
  ] as ComponentCard[];
  ...
  return {
    kind: report.audit!.kind,
    stats: [
      { value: `${formatNumber(summary.khm_top1_discriminating_runs)}/${formatNumber(summary.sensitivity_discriminating_runs)}`, label: "KHM top-one discriminating runs" },
      { value: `${formatNumber(summary.cambodia_heatwave_affected_rank)}/${formatNumber(summary.heatwave_major_rows_n)}`, label: "Cambodia affected-count rank" },
      { value: signed(indexCorrelation.spearman), label: "proxy vs affected count" },
    ],
    chartTitle: "The formula survives five runs; the observed order does not.",
    chartDeck: "The run ledger names the all-zero tie and Pakistan-leading case. The construct cards then show the rank reversal, near-zero proxy correlation, demographic scale, and missing school-day join.",
    laneRows,
    componentCards,
    readouts: [
      { label: "UNICEF ADB annex rows", value: formatNumber(summary.unicef_adb_annex_rows_n) },
      { label: "Old panel x UNICEF overlap", value: formatNumber(summary.old_panel_unicef_overlap_n) },
      { label: "Heatwave-major rows", value: formatNumber(summary.heatwave_major_rows_n) },
      { label: "Heatwave-major affected students", value: formatNumber(summary.heatwave_major_students_affected) },
      { label: "Complete enrollment overlap", value: formatNumber(summary.complete_enrollment_overlap_n) },
      { label: "Child-population correlation", value: signed(populationCorrelation.spearman) },
      { label: "Decision", value: String(data.decision || "Retire the national disruption ranking.") },
    ],
    sourceFacts: sourceTrail,
    caveats: unique(baseCaveats(report, data).concat([
...
export default function ShowcaseEvidenceAudit({ reportSlug }: { reportSlug: string }) {
      ...
      <section className="showcase-section audit-evidence-section">
        <div className="showcase-explorer-head">
          ...
          <div className="audit-nav-card">
            <Link href={previousReport.href}>Previous report</Link>
            <Link href={nextReport.href}>Next report</Link>
          </div>
        </div>
      ...
      {model && (
        <section className="showcase-section showcase-two-col">
          ...
          <div className="showcase-source-box audit-source-box">
            ...
            <Link href={`/${report.audit.programSlug}`}>Open program evidence page</Link>
          </div>
        </section>
      )}
