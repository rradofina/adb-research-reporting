...
# When heat closes schools, which country ranks first?
...
UNICEF counted at least 242 million students in 85 countries whose schooling
was disrupted by climate events in 2024 [@unicef2025learninginterrupted]. The
World Bank separately documents the scale of climate-related education loss
and the need for adaptation [@worldbank2024choosingfuture]. Neither source
implies that a simple national index can identify where disruption is greatest.

A national screening index had ranked Cambodia first. This paper asks whether
that ranking survives its own sensitivity files and an observed disruption
object.
...
counts, with a bootstrap interval from +0.52 to +1.00. Multiplying scale by
two weakly validated national terms does not improve the ordering in this
subset.
...
# Why the proxy fails up close

## Test 1: does the robustness statement match the saved runs?

The public sentence said Cambodia remained first across every perturbation. The
committed run ledger is the right place to check.
...
ratio is capped at 40.

The committed run ledger contains seven specifications: Cambodia is first in
the baseline and four cap perturbations; Pakistan is first when the
temperature floor is reduced by 50%; and every economy receives zero when the
temperature floor is increased by 50%.
...
43-economy ADB roster, and 19 overlap the old 32-economy proxy panel. The six
rows with heatwave as the major hazard are Afghanistan, Bangladesh, Cambodia,
India, Pakistan, and the Philippines. Together they account for 154,888,029
affected students in the annex.
...
construction, and year; some students may also be counted across events.

# What this means for education and climate work
...
screen. The appropriate interpretation is not “no relationship exists.” Six
selected observations cannot establish that. The defensible statement is
narrower: this sample provides no rank-order validation for the inherited
index.
...
sampling frame for that pilot. Retiring the ranking replaces a confident but
unsupported country story with a reproducible construct test: the “every
perturbation” wording was wrong, one sensitivity run was non-discriminating,
and the observed 2024 heatwave subset does not preserve the proxy's order.
...
choices are arbitrary. For each of the six heatwave-major rows, the pipeline
joins the UNICEF count to the inherited index and its component variables. The
principal statistic is Spearman's rank correlation because neither the proxy
nor the counts support a linear scale interpretation. Each correlation carries
a deterministic 95% bootstrap interval based on 5,000 resamples. With only six
observations, these intervals are expected to be wide.

To expose scale and vintage issues, the pipeline retrieves each country's
latest available total enrollment in pre-primary, primary, and secondary
education between 2015 and 2025 through the World Bank Indicators API
[@worldbank2026indicatorsapi]. All three levels are available for 19 of the
21 UNICEF rows and 17 of the 19 rows that also have the old index. Because
level-specific years can differ, the resulting affected-to-enrollment
percentage is a diagnostic—not a harmonized disruption rate. The analysis
transcribes the 21 annex rows and programmatically checks each country, count,
and hazard against extracted text from the public PDF before using it.
...
Indicators API responses into the ignored program cache. Every reported number
and chart is regenerated from those artifacts. Retrieval dates, source URLs,
and file checksums are stored in the generated evidence object and
`versions.json`. Inspect the full evidence object at
[/program/school-heat-disruption/evidence](/program/school-heat-disruption/evidence).
