...
export const showcaseReportDepth: Record<number, ShowcaseReportDepth> = {
  ...
  6: {
    ...
    limitation: "Ground-monitor visibility is not pollution ranking, compliance assessment, or monitor siting adequacy; the reconciliation audit has candidate rows but zero validated joins, the exact-row method audit finds raw live-value issues, the Uzbekistan station-specific scan closes the station-ID gate through official detail URLs, the status/certification source scan adds context but still lacks explicit current-status and grade closure, and complete monitor-grade classification plus catchment coverage remain at zero.",
  },
  7: {
...
export const showcaseReportQuality: Record<number, ShowcaseReportQuality> = {
  ...
  6: {
    ...
    nextUpgrade: "Find public correction/status evidence for Uzbekistan station IDs 107, 728, and 737, then resolve the 38 Indonesia/Georgia PM2.5 portal/API rows without promoting any catchment denominator until grade evidence is complete.",
  },
  7: {
    readiness: "l3-candidate",
    readinessLabel: "L3 candidate",
