...
PANEL = BASE / "generated" / "migration-displacement-adb-panel.json"
POP_CACHE = BASE / ".cache" / "wdi-population-2024.json"
OUT = BASE / "generated"
...
POP_URL = (
    ...
    f"{POP_INDICATOR}?format=json&date={POP_YEAR}&per_page=20000"
)


def fetch_population_payload():
    """Fetch the fixed WDI query, falling back to the program-local cache."""
    POP_CACHE.parent.mkdir(parents=True, exist_ok=True)
    try:
        request = urllib.request.Request(
            POP_URL,
            headers={"Accept": "application/json", "User-Agent": "adb-research-factory/1.0"},
        )
        with urllib.request.urlopen(request, timeout=90) as response:
            raw_bytes = response.read()
        POP_CACHE.write_bytes(raw_bytes)
        fetch_mode = "live"
    except (urllib.error.URLError, TimeoutError):
        if not POP_CACHE.exists():
            raise
        raw_bytes = POP_CACHE.read_bytes()
        fetch_mode = "cache fallback"
    return json.loads(raw_bytes.decode("utf-8-sig")), {
        "url": POP_URL,
        "cache_path": str(POP_CACHE.relative_to(BASE)),
        "bytes": len(raw_bytes),
...
def load_population():
    """Return iso3 -> population and a public-source retrieval record."""
    raw, retrieval = fetch_population_payload()
    data = raw[1]
    ...
    for r in data:
        ...
        v = r.get("value")
        if iso and v is not None and r.get("date") == POP_YEAR:
            pop[iso] = (int(round(v)), POP_YEAR)
    retrieval["indicator"] = indicator
    retrieval["year"] = POP_YEAR
    retrieval["lastupdated"] = raw[0].get("lastupdated") if raw else None
    retrieval["retrieved_at"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    return pop, retrieval


def main():
    panel = json.loads(PANEL.read_text(encoding="utf-8"))
    rows = panel["rows"]
    pop, pop_retrieval = load_population()
    ...
    payload = {
        ...
        "claim_scope": (
            "Denominator-switch audit of the absolute emigrant-stock screen. Recomputes the "
            ...
            "panel). Denominator: World Bank WDI SP.POP.TOTL mid-year 2024 from a "
            "fixed public API query with a program-local cache. "
        ...
        "sources": {
            ...
            "denominator": {
                ...
                "indicator": pop_retrieval["indicator"],
                "year": POP_YEAR,
                "license": "CC BY 4.0",
                "retrieval": pop_retrieval,
            },
        },
    ...
    with (OUT / "migration-per-population-deepening.json").open("w", encoding="utf-8") as f:
    ...
    with (OUT / "migration-per-population-deepening.csv").open("w", encoding="utf-8", newline="") as f:
