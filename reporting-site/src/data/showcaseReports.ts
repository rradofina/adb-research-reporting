...
export const showcaseReportDepth: Record<number, ShowcaseReportDepth> = {
  ...
  6: {
    ...
    limitation: "Ground-monitor visibility is not pollution ranking, compliance assessment, or monitor siting adequacy; the reconciliation audit has candidate rows but zero validated joins, the exact-row method audit and decision ledger find raw-value and blocker issues, the Uzbekistan station-specific scan closes the station-ID gate through official detail URLs, the status/certification, blocker, and endpoint-consistency scans add context but still lack explicit current-status and grade closure, the Indonesia/Georgia and station-code scans add exact public row context without grade closure, BMKG station/API telemetry is visible but lacks status/certificate fields, the Georgia report/export ladder keeps verified-report closure at zero, the Georgia verification-policy wall confirms the policy route but not a station-status or verified-closure route, the Georgia report-frequency matrix finds no daily, monthly, annual, XLSX, or PDF closure route, the Georgia NEA network/launch scan adds city-level context but no station-code/status/calibration/grade closure, and complete monitor-grade classification plus catchment coverage remain at zero.",
  },
  7: {
...
export const showcaseReportQuality: Record<number, ShowcaseReportQuality> = {
  ...
  6: {
    ...
    nextUpgrade: "For Uzbekistan, find a public station-owner or regulator source that names station IDs 107, 728, or 737 with correction/status/calibration or grade-basis closure; for BMKG, move beyond station studies, PM2.5 installation/deployment context, source-level operational routes, and source-level grade-basis rules by finding station-specific inspection logs, calibration certificates, calibration-status records, or explicit station-grade records; for Georgia, find a station-specific verified report/status/method route beyond the policy note, daily/monthly caution routes, annual server-error routes, XLSX station-sheet-only exports, and NEA city-level network/launch context before any catchment denominator is promoted.",
  },
  7: {
    readiness: "l3-candidate",
    readinessLabel: "L3 candidate",
