...
Status: **§18 AI-first PR-pending-DOI — 2026-04-25.**
Two-DMC pilot (PHL + BGD). Sensitivity at ±50% complete in both DMCs
(no critical failures). Internal review (`review-internal.md`) and
external red-team review (`review-external.md`) closed under
`CONSTITUTION.md` §18.1 and §18.4 — both AI-attested. The Zenodo DOI
is the only remaining gate artifact and is owner-only per §18.1
(non-suspendable).
...
## Philippines poverty-context amendment (2026-05-05)

The first Philippines poverty-context join now exists at
`generated/psdq-phl-admin3-poverty-context.{csv,json}`. The preferred PSA 2023
city/municipality Small Area Estimates (SAE) Excel attachment was manually
downloaded by the owner from the official PSA page on 2026-05-05 and seeded
into the deterministic cache. Scripted/static-file attempts still record
Cloudflare's managed browser challenge (`cf_mitigated: challenge`), so the
manual source-acquisition step is documented in `SOURCE-ACTION.md`.
...
| Rows joined from PSA 2023 city/municipal SAE Excel | 1,597 |
| Rows joined from PSA OpenSTAT 2023 direct-estimate table | 35 |
| Rows still missing SAE/direct poverty value | 10 |

Top rows by the poverty-context screen are Zamboanga City, Davao City,
Cotabato City, Cagayan de Oro City, Butuan City, City of Mati, Iligan City,
Sindangan, and Datu Odin Sinsuat. The screen multiplies the existing ADM3
registry-map gap building proxy by official poverty incidence only where a
poverty source joined. It is **not** affected population, demand, welfare loss,
or a substitute for household microdata.
