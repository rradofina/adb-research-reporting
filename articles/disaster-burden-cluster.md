...
# After the storm, the clock that will not start

When a major storm hits, recovery language arrives quickly: how many months
until lights, services, or livelihoods return. Country disaster tallies and
daily nighttime radiance look like natural inputs to that clock. This paper
asks whether those public objects yet support a defensible recovery-month
measure for planning or ranking.
...
# What we found: burden disagrees, and no stable recovery month
...
# Why burden cannot answer a recovery question—and Haiyan up close
...
small island economies. That last comparison is deliberately labelled
indicative: it divides 2000–2025 events by 2024 population. It demonstrates
scale dependence; it is not a final annualized exposure rate.
...
# What this means for anyone building a recovery measure
...
account. Access, however, is only one validity check.
...
This is not evidence that places failed to recover. It is evidence that this
measurement chain cannot yet say when they did. Country burden totals are not a
common severity scale, and administrative centroids are approximations of impact
zones, not verified damage footprints.

Raw background-subtracted radiance can be negative; dark baselines produce
volatile ratios; the plotted series is clipped only to remain readable. The
main-specification months are candidate outputs, not headlines. The geometry
screen is a gross-error filter only. Radiance is not welfare, not electricity
restoration, not livelihood recovery, and not a causal policy effect.

No individual external reviewer was contacted. Human-final review remains
owner-led. Maturity remains PP under `attestation_chain: ai-first`.
...
Any one of those objects would change how far a recovery-month claim can travel.
Until they arrive, the public archive remains accessible and the proposed
recovery construct remains unvalidated.
...
geography, repeated observations, and an interpretable outcome. Committed
scripts extract the fixed-orbit panel, apply quality filters, run the full
sensitivity matrix, screen candidate centroids against Natural Earth polygons,
and render the figure dossier in the program evidence package.
