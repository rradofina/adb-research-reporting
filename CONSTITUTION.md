...
| 1 | When the facility map changes the rank | `access-services/` (refresh; legacy at `luminosity-gap/research/access-services/`) | **PP — label corrected 2026-08-05** (the top-4 {BGD, KHM, LAO, PAK} access headline was retired in the 2026-07-18 claim-reshape pass; the retained result is that official clinical registries reorder 16 of 17 Philippine regional people-per-facility ranks and 6 of 8 Bangladesh division ranks, with OSM capture and apparent load at Spearman −0.81; `/program/access-services/evidence`; `attestation_chain: ai-first`) | 18/30 (AI under §18) | Adofina | 2026-08-05 |
...
| 3 | Public monitor QA observability | `air-monitoring/` (refresh; legacy at `luminosity-gap/research/air-monitoring/`) | **SR under §18 — documented-absence result** (claim text corrected 2026-08-05; the 2026-04-27 top-5 PM2.5 observability-gap set {AFG, BGD, MMR, UZB, TJK} is superseded by the 2026-07-19 finding: the generated ledger reads 64 committed summary rows and 214 supporting files, and records 0 validated same-station rows, 0 station-specific inspection/certificate/status rows, 0 complete monitor-grade rows, and 0 allowed coverage-claim rows; `/program/air-monitoring/evidence`; `attestation_chain: ai-first`) | 18/30 (AI under §18) | Adofina | 2026-08-05 |
| 4 | Urbanization depends on the definition | `invisible-urbanization/` (new pipeline; legacy stub at `luminosity-gap/research/invisible-urbanization/`) | **PP — label corrected 2026-08-05** (the rural-share × urban-pop-growth proxy and its {AFG, BGD, LAO, PNG, SLB} top five were retired on 2026-07-19; the retained measurement result is a median absolute 19.98-point GHSL–WDI urban-share gap across 40 complete 2020 cases, GHSL higher in 33 and WDI in 7, not converging across epochs from 1975; `/program/invisible-urbanization/evidence`; `attestation_chain: ai-first`) | 17/30 (AI under §18) | Adofina | 2026-08-05 |
...
| 8 | Flooded routes to market in Sylhet | `flood-market-access/` | **PP — label corrected 2026-08-05** (the national rural-share × flood-event composite and its {AFG, CHN, IDN, IND} top four were retired on 2026-07-19; the retained result is the Sylhet route object, where 345,718 modeled people — 41.24% of the baseline-accessible population — lose a route to eight mapped marketplaces, holding between 38.92% and 43.45% across 54 variants; `/program/flood-market-access/evidence`; `attestation_chain: ai-first`) | 17/30 (AI under §18) | Adofina | 2026-08-05 |
...
| 19 | Robust and wrong at the same time | `index-failure-modes/` | **PP — new program 2026-08-05** (no maturity claimed beyond prepared pipeline: the panel has run and carries empirical values, but `pre-registration.md`, `literature.md`, and `sensitivity.md` do not exist, so the SR gate artifacts are absent; cross-program synthesis of the twelve construct-validation tests this repository ran on orderings it built itself; in 5 of 5 paired cases the ±50% parameter suite certified a leading set as stable and a later construct check rejected the same set, with certified and tested sets verified identical in 4; reads only committed generated artifacts and measures nothing about any economy directly; the 2026-08-05 literature check found four of five results to be settled method or local defects; no public surface, no article) | not scored | Adofina | 2026-08-05 |

**Current WIP allocation under §8.1 (updated 2026-08-05):**
- Publication-ready (max 1, suspended under §18): public-service-data-quality.
- Screening result (max 3, suspended under §18): digital-performance,
  air-monitoring, public-data-freshness.
- **Label correction, 2026-08-05.** access-services, invisible-urbanization,
  and flood-market-access were carrying SR-under-§18 rows in this table whose
  claim text was the exact top-N ranking each program had already retired
  (2026-07-18 and 2026-07-19). Under `CLAUDE.md`, demoting a label that no
  longer reflects the artifact is an honesty correction and is AI-permitted;
  promotion remains gated. All three are now PP, matching
  `research/wip-register.md`, which had already placed them there. The
  air-monitoring row keeps SR — its program board defends the label — but its
  claim text was superseded and has been replaced with the current
  documented-absence result. The divergence survived because
  `scripts/check-wip.mjs` never parsed this table; that gap is now closed.
