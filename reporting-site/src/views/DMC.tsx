...
export default function DMC({ iso3 = "" }: { iso3?: string }) {
  const router = useRouter();
  const dmc = dmcByIso.get(iso3.toUpperCase());
  const canonicalDmc = DMCS.find((item) => item.iso3.toLowerCase() === iso3.toLowerCase());
  const [views, setViews] = useState<IndicatorView[] | null>(null);

  useEffect(() => {
    if (!dmc && canonicalDmc) {
      router.replace(`/dmc/${canonicalDmc.iso3}`);
    }
  }, [canonicalDmc, dmc, router]);
  ...
  return (
    <div className="reveal">
      ...
      <nav className="flex items-center justify-between gap-4 flex-wrap">
        <Link href="/atlas" className="ed-link text-sm uppercase tracking-[0.18em] font-mono">
          ← Back to atlas
        </Link>
    ...
    <li className="py-7">
      <div className="grid grid-cols-12 gap-4 items-start">
        ...
        <div className="col-span-12 sm:col-span-7">
          <Link href={def.href} className="group block">
