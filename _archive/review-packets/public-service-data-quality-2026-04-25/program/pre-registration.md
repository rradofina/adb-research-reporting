...
Status: **AI-drafted. Owner freezes by signing §10 below in a commit message before any further pipeline run.**

Governed by `CONSTITUTION.md` §3.2 and §7. This document is **frozen
before the next pipeline run**. The screening-result computations
already in `generated/` are treated as exploratory under §3.2; the
pre-registration applies to the next pipeline run, which will be the
input to the SR → PR gate.
...
| Field | Value |
|---|---|
| Frozen by | *(pending — owner signs)* |
| Date frozen | *(YYYY-MM-DD)* |
| Commit hash | *(hash)* |
| Pipeline run started after this commit | *(yes / no)* |

Once the owner signs §10 in a commit message, this document is the
pre-registration. Any change to §1–§9 after the next pipeline run is a
retraction of the prior result, not an edit. The new pre-registration
replaces this one and the prior result moves to `retracted/`.

## 11. AI assistance disclosure (§12, AI_TRANSPARENCY.md)

- This document is AI-drafted from `literature.md`, `results.md`,
  `pipeline.ts`, and `scripts/process-disagreement.py`. The owner is
  expected to read every line, change wording where needed, and freeze
  it with a written attestation in the commit message.
- AI may not freeze §10. Only the owner does.
