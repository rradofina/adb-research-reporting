...
export const showcaseReports: ShowcaseReport[] = [
  ...
  },
  {
    ...
    evidencePath: "air-monitoring/bmkg-api-parity-status.md",
...
export const showcaseReportDepth: Record<number, ShowcaseReportDepth> = {
  ...
  6: {
    ...
    limitation: "Ground-monitor visibility is not pollution ranking, compliance assessment, or monitor siting adequacy; the reconciliation audit has candidate rows but zero validated joins, the exact-row method audit and decision ledger find raw-value and blocker issues, the Uzbekistan station-specific scan closes the station-ID gate through official detail URLs, the status/certification, blocker, and endpoint-consistency scans add context but still lack explicit current-status and grade closure, the Indonesia/Georgia and station-code scans add exact public row context without grade closure, BMKG station/API telemetry is visible but lacks status/certificate fields, the Georgia report/export ladder keeps verified-report closure at zero, and complete monitor-grade classification plus catchment coverage remain at zero.",
  },
  7: {
