...
| Edited an evidence-review artifact or its evidence register (§2.7 track) | Run `verify_citations.py` — it must exit 0 — then `locate_estimates.py`. A number without a locator may sit in the register; it may not enter a headline, abstract, table, figure, annotated bibliography, or synthesis sentence |
