...
export const showcaseReportQuality: Record<number, ShowcaseReportQuality> = {
  ...
  6: {
    ...
    publicationGap: "Needs validated station crosswalk rows, station-level complete monitor-grade classification across sources, selected denominator files downloaded and checksummed from the prefreeze manifest, a resolved ACAG version decision, frozen radius/de-duplication/join/grade rules, and denominator validation before station-radius or catchment language.",
    nextUpgrade: "For Uzbekistan, find a public station-owner or regulator source beyond the Air Uzbekistan alternate namespace that names station IDs 107, 728, or 737, or explicitly crosswalks them to IDs 1, 20, and 26, with correction/status/calibration or grade-basis closure; for BMKG, target station-specific inspection logs, PM2.5 calibration certificates, calibration-status records, or explicit station-grade records beyond station pages, dashboard status, station studies, installation/deployment context, source-level rules, and the PPID/PTSP access-route wall; for Georgia, find a station-specific verified report/status/method route beyond the policy note, daily/monthly caution routes, annual server-error routes, XLSX station-sheet-only exports, NEA city-level network/launch context, and the indicator API's different station-code namespace; for station-radius work, choose a tractable GHSL/WorldPop file or tile subset and ACAG object from the prefreeze manifest, resolve whether current V6.GL.03 can replace or only supplement the source-plan V6.GL.02.04/V5 routes, freeze the radius/de-duplication/join/grade rules, then download and checksum only the selected denominator subset before any catchment map.",
  },
  7: {
    readiness: "l3-candidate",
    readinessLabel: "L3 candidate",
