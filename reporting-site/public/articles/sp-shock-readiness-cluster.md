...
# When a shock hits, who can get a payment?
...
An inherited country screen multiplied the World Bank poverty headcount by one
minus the average of ASPIRE social-protection coverage and Global Findex
account ownership, then named Bangladesh, Lao PDR, Myanmar, Pakistan, and the
Philippines as a stable shock-payment-readiness-gap top five. The research
question is not “which economies are least ready?” It is narrower and
falsifiable: does the inherited construction preserve its own stated ranking
rule, and does a qualified direct response source validate the interpretation
attached to that ranking?
...
# Why the ranking broke

The inherited screen had none of the event-level delivery objects. It combined
three national indicators:
...
for the two-leg mean and still computes (G_i). A one-legged record is scored,
but the published headline later selects only records with both legs.
...
# What this means for shock-response work
...
Four tests follow the reconstruction:
...
3. **Coverage-construct test.** Replace all-social-protection coverage with the
   narrower ASPIRE safety-net series and rerank.
4. **Observed-response test.** Compare the inherited gap with the count of
   response categories documented in the World Bank COVID-19 matrix.

The analysis reproduces the exact value for every economy with poverty data,
...
not appear in the parsed matrix. Response breadth is not relabeled readiness.
Spearman correlation is bootstrapped with 2,000 deterministic resamples. The
inherited component weights were already varied by ±50 percent; that exercise
tests internal formula sensitivity and cannot establish construct validity.
...
[/program/social-protection-shock-coverage/evidence](/program/social-protection-shock-coverage/evidence).
