...
export const showcaseReportDepth: Record<number, ShowcaseReportDepth> = {
  ...
  6: {
    ...
    limitation: "Ground-monitor visibility is not pollution ranking, compliance assessment, or monitor siting adequacy; the reconciliation audit has candidate rows but zero validated joins, the exact-row method audit finds raw live-value issues, the Uzbekistan station-specific scan closes the station-ID gate through official detail URLs, the status/certification and blocker scans add context but still lack explicit current-status and grade closure, the Indonesia/Georgia scan adds same-page BMKG method context and Georgia alias context without grade closure, and complete monitor-grade classification plus catchment coverage remain at zero.",
  },
  7: {
...
export const showcaseReportQuality: Record<number, ShowcaseReportQuality> = {
  ...
  6: {
    ...
    nextUpgrade: "Find station-code method/status tables or correction notes for the Indonesia/Georgia and Uzbekistan blocker rows without promoting any catchment denominator until grade evidence is complete.",
  },
  7: {
    readiness: "l3-candidate",
    readinessLabel: "L3 candidate",
