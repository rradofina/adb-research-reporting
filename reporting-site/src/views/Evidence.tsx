...
export default function Evidence({ slug = "" }: { slug?: string }) {
  ...
  if (missing) {
    return (
      <div className="py-20 text-center reveal">
        ...
        <Link href="/research" className="ed-link mt-6 inline-block">← Back to research index</Link>
      </div>
    );
  }
  if (!manifest) {
          ...
          <aside className="md:col-span-5 md:pl-8 md:border-l md:border-[var(--rule-soft)] marginalia space-y-3">
            ...
            {programHref && (
              <div className="pt-2 mt-3 border-t border-[var(--rule-soft)]">
                <Link href={programHref} className="ed-link font-mono text-xs uppercase tracking-[0.18em]">
                  ← Program detail
                </Link>
              </div>
            )}
            {article && (
              <div>
                <Link href={`/findings/${article.slug}`} className="ed-link font-mono text-xs uppercase tracking-[0.18em]">
      ...
      {overall === "ai-first" && (
        ...
        >
          ...
          <p className="text-ink-soft leading-relaxed max-w-prose">
            This packet is published under <Link href="/about#ai" className="ed-link">CONSTITUTION.md §18 (AI-First Operating Mode)</Link>.
