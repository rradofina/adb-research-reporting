...
- **2026-07-01 (air-monitoring station-radius claim-eligibility targets):**
  ...
  generated
  `air-monitoring/generated/air-monitoring-station-radius-claim-eligibility-targets.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-claim-eligibility-targets-summary.json`,
  and wrote `air-monitoring/station-radius-claim-eligibility-targets.md`.
  The no-network derivative reads the committed identity, grade, Georgia
  verification, Uzbekistan endpoint, and coverage-claim gates; records 5
  target lanes and 96 blocked row-lane checks; and converts the next loop
  blocker into required public-document primitives. The public air-monitoring
  route now renders this matrix after the coverage-claim gate. No rows are
  promoted and coverage-claim permission remains blocked.

- **2026-06-25 (air-monitoring station-identity validation gate):** Added
  `air-monitoring/scripts/build-station-identity-validation-gate.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-identity-validation-gate.csv`
  and
  `air-monitoring/generated/air-monitoring-station-identity-validation-gate-summary.json`,
  and wrote `air-monitoring/station-identity-validation-gate.md`. The
  derivative gate reads the candidate review worksheet summary, candidate
  crosswalk source scan, candidate public-feed source scan, and one-signal
  review queue; checks 44 identity candidates across 4 economies; separates 13
  source-screened near-plus-name rows from 31 one-signal identity rows; and
  finds 0 shared station IDs, 0 source-owner crosswalk rows, 0 current-status
  crosswalk rows, 0 documented co-location rows, 0 validated same-station
  rows, and 0 station-radius identity-ready rows. The downstream
  station-radius coverage-claim gate now reads this identity gate and still
  blocks all coverage language. The public route now renders the identity gate
  between the country-unioned catchment dry run and the coverage-claim gate.
- **2026-06-21 (air-monitoring station-radius coverage-claim gate):** Added
  `air-monitoring/scripts/build-station-radius-coverage-claim-gate.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-coverage-claim-gate.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-coverage-claim-gate-summary.json`,
  and wrote `air-monitoring/station-radius-coverage-claim-gate.md`. The
  derivative gate reads the denominator join, country-unioned catchment,
  station-identity, monitor-grade, BMKG, Uzbekistan, and Georgia blocker
  artifacts; checks 11 primary-radius economy rows, 33 unioned denominator
  rows, and 831 row-level GHSL/ACAG joins; and blocks all 11 country rows
  because validated same-station joins, complete monitor-grade rows,
  station-radius-ready economies, and claim-allowed rows remain at 0. The
  public route now renders the claim gate immediately after the
  country-unioned overlap wall.
- **2026-06-21 (air-monitoring station-radius country-unioned catchment dry
  run):** Added
  `air-monitoring/scripts/build-station-radius-country-unioned-catchment-dry-run.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-country-unioned-catchment-dry-run.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-country-unioned-catchment-dry-run-summary.json`,
  and wrote
  `air-monitoring/station-radius-country-unioned-catchment-dry-run.md`. The
  dry run reads the committed row-level denominator join outputs, reuses the
  checked GHSL/ACAG custody helpers, counts each GHSL population cell at most
  once within each economy and source-frozen radius band, and computes 33
  country-radius union rows, 33 GHSL unioned population denominators, and 22
  ACAG union-cell PM2.5 context rows. At the primary 4 km band, Bangladesh
  changes from an 80.227 million row-buffer denominator to a 24.712 million
  country-unioned denominator, making duplicate candidate-buffer mass visible
  while keeping validated same-station joins, complete monitor-grade rows,
  station-radius-ready economies, and coverage-claim permission at 0/false.
  The public route now renders a country-unioned overlap wall after the
  row-level denominator join wall.
...
- **2026-06-20 (air-monitoring BMKG regional status/source scan):**
  Added and then expanded
  `air-monitoring/source-inputs/bmkg-regional-status-source-seed.csv` and
  `air-monitoring/scripts/scan-bmkg-regional-status-sources.py`, generated
  ...
  and wrote `air-monitoring/bmkg-regional-status-source-scan.md`. The
  networked scan tests 22 BMKG rows against 10 public regional BMKG status and
  analysis pages, public-information, service, PPID, and regulator sources
  outside the central station-detail/API surfaces. It retrieves all 10
  sources, finds 5 rows with exact station-name or official site-variant
  external context, records 3 rows with regional-analysis context, confirms 1
  Banjarbaru row with official regional `ONLINE` status and a recent
  timestamp, and keeps station-specific inspection-log,
  calibration-certificate/status, complete monitor-grade, and
  station-radius-ready rows at 0. The public route renders a BMKG regional
  status/analysis wall. Production build passed with the existing chunk-size
  warning. Focused Playwright QA passed at 1440x1100 and 390x1000 with 6 stat
  cards, 3 decision lanes, 5 evidence rows, 10 gate cards, 10 source cards, 3
...
- **2026-06-20 (air-monitoring BMKG station public-context source scan):**
  Added `air-monitoring/source-inputs/bmkg-station-public-context-source-seed.csv`
  and `air-monitoring/scripts/scan-bmkg-station-public-context-sources.py`,
  generated
  ...
  and wrote `air-monitoring/bmkg-station-public-context-source-scan.md`. The
  networked scan retrieves 9 public station-unit, academic/journal, regulator,
  and deployment-context sources for the 22 BMKG BAM-classified target rows. It
  records 9 rows with public station or deployment context, 4 station-unit or
  ...
  certificate/status records, current-status confirmations from this pass,
  complete monitor-grade rows, and station-radius-ready rows at 0. The public
  route now renders a BMKG station public-context source wall. Production build
  passed with the existing chunk-size warning. Focused Playwright QA passed at
  1440x1100 and 390x1000 with 6 stat cards, 4 decision lanes, 9 matched row
  cards, 9 source cards, 7 gate cards, 3 working download links, required text
...
- **2026-06-20 (air-monitoring BMKG installation/audit source scan):** Added
  ...
  and wrote `air-monitoring/bmkg-installation-audit-source-scan.md`. The
  networked scan retrieves 6 official BMKG installation, audit/calibration,
  public-information, and operational-monitoring sources for the 22 BMKG
  BAM-classified target rows. It records 8 rows with installation or audit
  context, 1 exact station audit/calibration context row, 7 PM2.5
  installation/deployment context rows, and 4 source-level operational or
  calibration routes. It keeps station-specific inspection logs,
  station-specific calibration certificate/status records, current-status
  confirmations from this pass, complete monitor-grade rows, and
  station-radius-ready rows at 0. The public route now renders a BMKG
  installation/audit source wall. Production build passed with the existing
  chunk-size warning. Focused Playwright QA passed at 1440x1100 and 390x1000
...
- **2026-06-20 (air-monitoring station-radius denominator source plan):**
  Added
  `air-monitoring/source-inputs/station-radius-denominator-source-seed.csv`
  and `air-monitoring/scripts/scan-station-radius-denominator-sources.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-denominator-source-plan.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-denominator-source-plan-summary.json`,
  and wrote `air-monitoring/station-radius-denominator-source-plan.md`. The
  source-plan scan retrieves 7 of 7 public source pages, classifies 2
  population denominator candidates, 2 PM2.5 denominator candidates, 2
  context-only WHO sources, and 1 boundary-reference source, while recording 0
  committed population rasters, 0 committed PM2.5 grids, 0 validated
  same-station joins, 0 complete monitor-grade rows, and 0
  station-radius-ready economies. This closes the public source-route question,
  not the catchment map or exposure computation.
- **2026-06-20 (air-monitoring station-radius denominator acquisition routes):**
  Added
  `air-monitoring/scripts/scan-station-radius-denominator-acquisition-routes.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-denominator-acquisition-routes.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-denominator-acquisition-routes-summary.json`,
  and wrote
  `air-monitoring/station-radius-denominator-acquisition-routes.md`. The scan
  re-fetches the 7 source-plan pages, finds visible acquisition routes for 4 of
  4 candidate denominator sources, records 87 visible route links, 65 cloud or
  listing routes, 21 context routes, 33 HEAD probes, and 20 probe-OK responses,
  while keeping exact denominator file URLs, committed population rasters,
  committed PM2.5 grids, validated same-station joins, complete monitor-grade
  rows, and station-radius-ready economies at 0. This closes the route
  visibility question, not the file-manifest, checksum, map, or exposure
  computation.
- **2026-06-20 (air-monitoring station-radius denominator file-manifest
  prefreeze):** Added
  `air-monitoring/scripts/build-station-radius-denominator-file-manifest-prefreeze.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-denominator-file-manifest-prefreeze.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-denominator-file-manifest-prefreeze-summary.json`,
  and wrote
  `air-monitoring/station-radius-denominator-file-manifest-prefreeze.md`. The
  prefreeze records 12 manifest rows, 10 exact public file/object records, 5
  exact population file records, 4 current ACAG V6.GL.03 AWS PM2.5 object
  records with source-plan version drift, 1 context metadata file record, and 2
  ACAG Box shared-folder routes that are still not exact file manifests. It
  keeps denominator downloads, SHA-256 denominator checksums, validated
  same-station joins, complete monitor-grade rows, station-radius maps, and
  station-radius-ready economies at 0. This closes the exact visible-file
  ledger question, not the selected-download, checksum, join, grade, map, or
  exposure computation.
- **2026-06-20 (air-monitoring station-radius denominator download
  feasibility):** Added
  `air-monitoring/scripts/build-station-radius-denominator-download-feasibility.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-denominator-download-feasibility.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-denominator-download-feasibility-summary.json`,
  and wrote
  `air-monitoring/station-radius-denominator-download-feasibility.md`. The gate
  classifies the 12-row prefreeze manifest into 4 first-wave checksum
  candidates, 2 conditional ACAG V6.GL.03 coarse PM2.5 candidates, 2 metadata
  or route-test candidates, 4 deferred multi-gigabyte population archives, 2
  deferred moderate/large PM2.5 objects, and 2 unresolved ACAG Box shared-folder
  routes, while keeping selected population denominators, downloads, SHA-256
...
- **2026-06-20 (air-monitoring station-radius ACAG coarse checksum gate):**
  Added
  `air-monitoring/scripts/download-station-radius-acag-coarse-checksums.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-acag-coarse-checksums.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-acag-coarse-checksums-summary.json`,
  and wrote `air-monitoring/station-radius-acag-coarse-checksums.md`. The
  gate downloads only the two approved 2023 V6.GL.03 coarse PM2.5 NetCDF
  objects into the ignored local cache, records 2 SHA-256 hashes, verifies 2
  size matches against S3 metadata, opens 2 NETCDF4 files, and records
  `PM25(lat,lon)` plus `lat`/`lon` coordinate variables for both files. It
  keeps population denominator selection and downloads, PM2.5 exposure rows,
  station catchments, validated same-station joins, complete monitor-grade
  rows, station-radius maps, and station-radius-ready economies at 0. This
  closes PM2.5 file custody for the pilot lane, not the population denominator,
  join, grade, map, or exposure computation.
- **2026-06-20 (air-monitoring station-radius GHSL population tile-selection
  gate):** Added
  `air-monitoring/scripts/select-station-radius-ghsl-population-tiles.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-population-tile-selection.csv`,
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-population-tile-selection-country.csv`,
  and
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-population-tile-selection-summary.json`,
  and wrote
  `air-monitoring/station-radius-ghsl-population-tile-selection.md`. The gate
  reads committed OpenAQ station-coordinate rows, official PM2.5
  station-coordinate rows, and the denominator-readiness lanes; builds a
  GHSL 2020 4326 3 arc-second selected tile queue using a 50 km draft-radius
  buffer; records 11 coordinate-ready economies, 277 coordinate rows, 23
  selected tile URLs, 23 HEAD probes, 7 HEAD-OK responses, 16 HEAD failures,
  and a 189.406 MB known-size subset from successful HEAD responses. It keeps
  GHSL ZIP body downloads, population SHA-256 checksums, GeoTIFF transform
  inspections, station-radius population rows, PM2.5 exposure rows,
  same-station joins, complete monitor-grade rows, station-radius maps, and
  station-radius-ready economies at 0. This closes the selected population
  URL queue, not population file custody, raster inspection, join, grade, map,
  or exposure computation.
- **2026-06-20 (air-monitoring station-radius GHSL population tile
  checksum/transform gate):** Added
  `air-monitoring/scripts/download-station-radius-ghsl-population-tiles.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-population-tile-checksums.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-population-tile-checksums-summary.json`,
  and wrote
  `air-monitoring/station-radius-ghsl-population-tile-checksums.md`. The gate
  reads the selected GHSL tile queue, downloads only selected HEAD-OK rows
  under the 60 MB first-wave threshold, stores raw ZIPs in the ignored
  `.cache/station-radius-ghsl-population-tiles/` directory, records 4
  downloaded population ZIP files, 4 SHA-256 hashes, 102.118 MB cached, 4 ZIP
  files opened, 4 GeoTIFF transforms inspected, 0 transforms matching the
  simple 10-degree routing assumption, 4 transform mismatch rows, 3 unresolved
  first-wave download candidates, 16 selected HEAD-not-OK tile blockers, and
  0 station-radius population rows, PM2.5 exposure rows, same-station joins,
  complete monitor-grade rows, maps, or station-radius-ready economies. This
  closes first-wave file custody for four tiles and opens a routing-method
  correction wall before any catchment computation.
- **2026-06-20 (air-monitoring station-radius GHSL tile-routing correction
  gate):** Added
  `air-monitoring/scripts/derive-station-radius-ghsl-tile-routing-correction.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-tile-routing-correction.csv`,
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-tile-routing-correction-country.csv`,
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-tile-routing-correction-origin.csv`,
  and
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-tile-routing-correction-summary.json`,
  and wrote `air-monitoring/station-radius-ghsl-tile-routing-correction.md`.
  The gate derives an observed GHSL R/C origin from 4 opened GeoTIFF bounds
  (north origin 89.09958351, west origin -180.00791704; both origin ranges
  0.00000008 degrees), reruns the same 50 km draft coordinate buffer across
  277 coordinate rows, changes the selected population queue from 23 previous
  tile URLs to 21 corrected tile URLs, retains 20 previous tile IDs, adds
  `R9_C32`, removes `R10_C28`, `R6_C23`, and `R6_C26`, keeps 6 corrected
  tiles with prior HEAD-OK evidence, 14 with prior HEAD-not-OK evidence, and
  1 newly added tile with unknown HEAD evidence, retains 3 downloaded ZIPs
  under corrected routing, flags 1 downloaded ZIP as removed under corrected
  routing, and keeps station-radius population rows, PM2.5 exposure rows,
  same-station joins, complete monitor-grade rows, maps, and
  station-radius-ready economies at 0. This closes the routing correction
  method gate, not corrected-queue tile custody, catchment computation, join,
  grade, map, or exposure computation.
- **2026-06-20 (air-monitoring station-radius GHSL corrected population tile
  custody gate):** Added
  `air-monitoring/scripts/download-station-radius-ghsl-corrected-population-tiles.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-corrected-population-tile-custody.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-corrected-population-tile-custody-summary.json`,
  and wrote
  `air-monitoring/station-radius-ghsl-corrected-population-tile-custody.md`.
  The gate reads the corrected 21-tile GHSL population queue, re-probes all 21
  corrected selected tile URLs, records 21 current HEAD-OK probes with size
  metadata, treats 18 tiles at or below the 60 MB first-wave threshold as
  eligible, reuses or downloads 18 ZIP files in the ignored
  `air-monitoring/.cache/station-radius-ghsl-population-tiles/` directory,
  records 18 SHA-256 hashes and 464.408 MB of cached ZIP data, opens 18
  ZIP/GeoTIFF payloads, finds 18 transforms matching corrected GHSL bounds and
  0 mismatches, and leaves `R7_C27`, `R7_C28`, and `R8_C26` as deferred large
  selected tiles. This closes corrected first-wave file custody, not the
  large-tile custody, catchment computation, join, grade, map, or exposure
  computation; station-radius population rows, PM2.5 exposure rows,
  same-station joins, complete monitor-grade rows, maps, and
  station-radius-ready economies remain at 0.
- **2026-06-20 (air-monitoring station-radius GHSL large population tile
  custody gate):** Added
  `air-monitoring/scripts/download-station-radius-ghsl-large-population-tiles.py`,
  generated
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-large-population-tile-custody.csv`
  and
  `air-monitoring/generated/air-monitoring-station-radius-ghsl-large-population-tile-custody-summary.json`,
  and wrote
  `air-monitoring/station-radius-ghsl-large-population-tile-custody.md`. The
  gate reads the corrected-custody summary, targets the 3 large selected GHSL
  tiles deferred by the 60 MB first-wave rule (`R7_C27`, `R7_C28`, and
  `R8_C26`), re-probes the public URLs, downloads or reuses all 3 ZIP files in
  the ignored `air-monitoring/.cache/station-radius-ghsl-population-tiles/`
  directory, records 3 SHA-256 hashes and 521.841 MB of cached ZIP data, opens
  3 ZIP/GeoTIFF payloads, and finds 3 transforms matching corrected GHSL bounds
  with 0 mismatches. Joined with the first-wave custody gate, corrected
  population file custody is now 21 of 21 selected tiles. This is large
