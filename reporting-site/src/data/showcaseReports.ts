...
export const showcaseReportDepth: Record<number, ShowcaseReportDepth> = {
  ...
  6: {
    ...
    limitation: "Ground-monitor visibility is not pollution ranking, compliance assessment, or monitor siting adequacy; the reconciliation audit has candidate rows but zero validated joins, the exact-row method audit and decision ledger find raw-value and blocker issues, the Uzbekistan station-specific scan closes the station-ID gate through official detail URLs, the status/certification and blocker scans add context but still lack explicit current-status and grade closure, the Indonesia/Georgia and station-code scans add exact public row context without grade closure, the Georgia report/export ladder keeps verified-report closure at zero, and complete monitor-grade classification plus catchment coverage remain at zero.",
  },
  7: {
...
export const showcaseReportQuality: Record<number, ShowcaseReportQuality> = {
  ...
  6: {
    ...
    nextUpgrade: "For BMKG, move beyond visible station-page telemetry by finding station-specific inspection logs, calibration certificates, status records, or grade-basis evidence; for Georgia, find verified report/status evidence beyond the tested not-verified report/export surface; for Uzbekistan, resolve exact blockers/status/certification before any catchment denominator is promoted.",
  },
  7: {
    readiness: "l3-candidate",
    readinessLabel: "L3 candidate",
