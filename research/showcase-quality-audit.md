...
- PSDQ report #4 then received a targeted-row public-source confirmation
  pass. New script
  `public-service-data-quality/scripts/confirm-bgd-facility-public-map-targeted-rows.py`
  reads the 40-row targeted inspection CSV and retrieves public DGHS profile
  pages plus public OSM API records for all targeted inspection rows. It writes
  ...
  priority-1 rows, records DGHS token support for all 40 rows, finds 6 rows
  with live candidate-name score at or above 0.75, and closes or reclassifies
  0 rows. This is public-source confirmation, not human validation or row
  closure. Verification passed: targeted-row confirmation script rerun,
  program-script `py_compile`, production build, six deterministic gates,
  `git diff --check`, review packet and zip rebuild, and desktop/mobile
  browser QA with no page-level horizontal overflow and no page errors.
  Screenshots:
...
- PSDQ report #4 then received a public-source decision-ledger pass. New
  no-network script
  `public-service-data-quality/scripts/build-bgd-facility-public-source-decision-ledger.py`
  reads the 40-row targeted confirmation CSV/summary and writes
  ...
  The pass selects 16 reviewer decision rows: 4 source-repair rows, 3
  ...
  validation, row closure, or maturity promotion. Verification passed:
  decision-ledger script rerun, program-script `py_compile`, production build,
  six deterministic gates, `git diff --check`, review packet and zip rebuild,
  and desktop/mobile browser QA with no page-level horizontal overflow and no
...
- PSDQ report #4 then received a possible same-facility review. New
  no-network script
  ...
  The pass reviews 3 possible same-facility candidates. All 3 have DGHS
  profiles and OSM API records retrieved; 1 row has name score at least 0.95;
  all 3 candidates are at least 2 kilometers from the inspection point; and 0
  rows are allowed for closure, same-facility reclassification, or map-absence
  language. This is a no-contact evidence gate, not human validation, row
  closure, or maturity promotion. Browser QA passed at 1440x1100 and 390x900
  with 3 rendered cards, KPJ, Aichi, Chattogram, and `0 closed` visible, no
  page-level or card-level horizontal overflow, and no page errors or console
...
- PSDQ report #4 then received a priority name-conflict review. New
  no-network script
  ...
  The pass reviews 9 priority-1 name-conflict candidates. All 9 have DGHS
  profiles and OSM API records retrieved; 1 row has candidate name score at
  least 0.70; 6 candidates are at least 5 kilometers from the inspection
  point; 1 candidate is at least 10 kilometers from the inspection point; 4
  candidate names contain an admin place name; and the current artifacts
  contain 0 public alias/location sources. It allows 0 closures, 0
  same-facility reclassifications, and 0 map-absence uses. Browser QA passed at
  1440x1100 and 390x900 with 9 rendered cards, Pabna, Narsingdi, `0 alias
  source`, and `0 closed` visible, no page-level or card-level horizontal
  overflow, and no page errors or console messages. Screenshots:
...
- PSDQ report #4 then received a zero-OSM upazila observability review. New
  no-network script
  `public-service-data-quality/scripts/build-bgd-facility-zero-osm-upazila-observability-review.py`
  reads the exposure-ranked disagreement CSV, exposure summary JSON,
  targeted public-map inspection CSV, and decision-ledger summary JSON, then
  writes
  ...
  facility-level absence uses, and 0 coordinate corrections. Browser QA
  passed at 1440x1100 and 390x900 with 8 division cards, 10 top-upazila
  cards, 8 targeted-strip cards, Sonargaon, Chattogram, `0 closed`, and
  `0 facility absence uses` visible, no page-level or card-level horizontal
...
- PSDQ report #4 then received a lower-priority name-conflict spot-check
  review. New no-network script
  `public-service-data-quality/scripts/build-bgd-facility-lower-priority-name-conflict-review.py`
  reads the targeted-row confirmation CSV and decision-ledger summary JSON,
  then writes
  ...
  The pass reviews 6 deferred lower-priority name-conflict candidates. All 6
  have DGHS profile and OSM API evidence retrieved; 4 rows share reused
  ...
  contain 0 public alias/location sources. It allows 0 closures, 0
  same-facility reclassifications, and 0 map-absence uses. Browser QA passed
  at 1440x1100 and 390x900 with 6 row cards, 4 candidate-cluster cards,
  `momotaz clinic`, `Broadbank Clinic Quatere`, `0 alias source`, and
  `0 map absence uses` visible, no page-level or card-level horizontal
  overflow, and no page errors. Screenshots:
  ...
  `reporting-site/qa/showcase-psdq-lower-name-conflict-review-mobile.png`, and
  `reporting-site/qa/showcase-psdq-lower-name-conflict-review-mobile-cards.png`.
- PSDQ report #4 then received a consolidated human-gated handoff matrix. New
  no-network script
  `public-service-data-quality/scripts/build-bgd-facility-human-gated-handoff.py`
  reads the source-repair clarification packet, possible same-facility review,
  priority and lower-priority name-conflict reviews, and zero-OSM
  observability summary, then writes
  ...
  The pass consolidates 39 open rows across 5 handoff groups and 15 upazilas:
  3 source-repair clarifications, 3 possible same-facility rows, 9 priority
  ...
  source-owner action while allowing 0 closures, 0 same-facility
  reclassifications, 0 map-absence uses, and 0 coordinate corrections.
  Browser QA passed at 1440x1100 and 390x900 with 5 group cards, 10 upazila
  cards, 12 handoff row cards, Durgapur, `0 closed`, and `0 map absence uses`
  visible, no page-level or card-level horizontal overflow, and no page
  errors or console messages. Screenshots:
...
- PSDQ report #4 then received a no-contact human-validation worksheet. New
  no-network script
  `public-service-data-quality/scripts/build-bgd-facility-human-validation-worksheet.py`
  reads the 39-row human-gated handoff CSV, then writes
  ...
  role classes; leaves 39 human-validation status fields and 39 proposed
  ...
  rows, and 0 prefilled coordinate-correction rows. Browser QA passed at
  1440x1100 and 390x900 with worksheet note and CSV links visible, no
...
- PSDQ report #4 then received a no-contact AI closure audit. New no-network
  script
  `public-service-data-quality/scripts/build-bgd-facility-ai-closure-audit.py`
  reads the 39-row human-validation worksheet, then writes
  `generated/psdq-bgd-facility-validation-ai-closure-audit.csv` and
  `generated/psdq-bgd-facility-validation-ai-closure-audit-summary.json`. The
  pass audits 39 rows across 5 handoff groups and 15 upazilas; 39 rows remain
  at the human- or source-owner wall; 0 rows are AI-actionable without human
  or source-owner evidence; and 39 rows are keep-open only. Browser QA passed
  at 1440x1100 and 390x900 with 6 gate cards, 4 wall cards, 10 upazila cards,
  12 row cards, audit note and CSV links visible, no page-level or
  section-level horizontal overflow, no page errors, and only existing React
  Router development warnings. Screenshots:
...
- Air-monitoring report #6 received a metadata-readiness wall on
  2026-06-19. New no-network script
  `air-monitoring/scripts/build-metadata-readiness-audit.py` reads the
  committed country panel and concentration/GDP-confound deepening, then
  writes `generated/air-monitoring-metadata-readiness-audit.csv` and
  `generated/air-monitoring-metadata-readiness-audit-summary.json`. The public
  route now renders 7 evidence gate cards, 4 summary cards, 10 upgrade-queue
  cards, and audit note/JSON/CSV links. The audit records 24 upgrade-queue
  rows and 0 station-coordinate, monitor-grade, station-vintage, or
  regulatory-inventory rows, so station-radius/catchment claims remain
  blocked. Browser QA passed at 1440x1100 and 390x900 with no page or
  metadata-section horizontal overflow, no page errors, and only existing
...
- Air-monitoring report #6 then received an official station-source extraction
  pass. New script
  `air-monitoring/scripts/extract-regulator-station-evidence.py` reads the
  ...
  station name-only rows, 1 count-only row, and 2 plan-count-only rows; 22 of
  the coordinate rows fall within 5 kilometers of an OpenAQ PM2.5 row as a
  screening diagnostic. The public route now renders the official-source
  reconciliation wall with 5 stat cards, 5 coordinate bars, 4 limited-evidence
  rows, 9 country cards, 6 evidence gates, and 3 download links. Chrome CDP QA
  at 1440x1100 and 390x1000 found no page or section horizontal overflow and
  no console/page errors. Screenshots:
  `reporting-site/qa/showcase-air-regulator-station-desktop.png`,
  `reporting-site/qa/showcase-air-regulator-station-mobile.png`, and
...
- Air-monitoring report #6 then received an official-to-OpenAQ reconciliation
  audit. New script
  `air-monitoring/scripts/reconcile-official-openaq-stations.py` reads the
  official station-source extraction and OpenAQ station metadata artifacts,
  then writes
  ...
  near-only candidate rows, 22 name-only-not-near candidate rows, 186 official
  coordinate rows without either candidate signal, 69 OpenAQ rows not used as
  a near candidate, and 0 validated same-station joins. The public route now
  renders the official/OpenAQ reconciliation ladder with 5 stat cards, 4 lane
  cards, 5 country cards, 6 evidence gates, and 3 download links. Chrome CDP
  ...
  `reporting-site/qa/showcase-air-official-openaq-mobile-gates.png`. This is
  a candidate reconciliation queue, not a validated station crosswalk and not
  station-radius population coverage.
- Air-monitoring report #6 then received an official/OpenAQ candidate review
  worksheet. New no-network script
  `air-monitoring/scripts/build-official-openaq-candidate-review.py` reads the
  ...
  The worksheet filters the 13 near-plus-name candidate rows across 4
  economies and adds source metadata, review questions, allowed decisions, and
  minimum validation evidence. It records 0 station-ID crosswalk rows, 0
  public current-status confirmation rows, 0 validated same-station joins, and
  0 station-radius-ready rows. This is a review queue, not station validation
  and not station-radius population coverage. The public route now renders the
  candidate-review worksheet panel; Chrome CDP QA passed at 1440x1100 and
  390x1000 with 4 stat cards, 4 review-flow cards, 4 country cards, 6 row
  cards, 5 evidence gates, 3 download links, no page or section horizontal
  overflow, and no console/page errors. Screenshots:
...
- Air-monitoring report #6 then received a public-evidence attachment for the
  candidate worksheet. New no-network script
  ...
  The audit records 13 OpenAQ owner/provider rows, 6 OpenAQ `isMonitor` true
  rows, 7 rows not marked `isMonitor`, 11 first-seen rows, 11 last-seen rows,
  0 exact station-ID overlaps, 0 exact official-agency owner/provider matches,
  0 explicit crosswalk rows, 0 validated same-station joins, and 0
  station-radius-ready rows. This sharpens the queue but does not validate a
  station crosswalk. The public route now renders the candidate public-evidence
  panel; Chrome CDP QA passed at 1440x1100 and 390x1000 with 5 public-evidence
  stat cards, 2 evidence-lane cards, 4 country cards, 6 row cards, 5 evidence
  gates, 3 download links, no horizontal overflow, and no console/page errors.
  Screenshots:
...
- Air-monitoring report #6 then received a candidate crosswalk public-source
  scan. New seed file
  ...
  fetch five public source URLs and review the 6 OpenAQ `isMonitor` candidate
  rows. The scan retrieves all 5 source URLs, finds official coordinate
  evidence for the 2 Bangladesh SPARTAN-adjacent rows, finds Uzhydromet
  public-map address evidence for the 4 Uzbekistan StateAir-adjacent rows,
  screens all 6 rows as separate nearby stations, and keeps validated
  same-station joins and station-radius-ready rows at 0. This is row-level
  source screening, not a catchment layer. The public route now renders the
  source-scan panel; Chrome CDP QA passed at 1440x1100 and 390x1000 with 5
...
- Air-monitoring report #6 then received a monitor-grade evidence audit. New
  ...
  station-radius population coverage.
- Air-monitoring report #6 then received a monitor-grade source-validation
  scan for the non-Bangladesh provenance-only lane. New script
  `air-monitoring/scripts/scan-monitor-grade-source-validation.py` reads
  `air-monitoring/source-inputs/monitor-grade-source-validation-seed.csv` and
  the one-signal queue, retrieves all 14 seeded public URLs across 7
  economies, covers all 138 monitor-grade provenance-only rows, and writes
  ...
  The scan finds 2 method/equipment context source rows, 5 standard/method
  context source rows, 6 official/automatic context-only source rows, 1 caution
  source row, 0 complete monitor-grade classification rows, and 0
  station-radius grade-assumption-ready rows. The public route now renders a
  source-validation wall with 5 stat cards, 7 country cards, 14 source cards,
  5 evidence gates, and 3 download links. Chrome CDP QA at 1440x1100 and
  390x1000 found no page or section horizontal overflow, no text overflow, and
  no page errors beyond existing React Router future-flag warnings.
  Screenshots:
  `reporting-site/qa/showcase-air-grade-source-desktop.png`,
  `reporting-site/qa/showcase-air-grade-source-mobile.png`, and
  `reporting-site/qa/showcase-air-grade-source-mobile-sources.png`. This is
  source-level method context, not station-level current grade certification
  and not station-radius population coverage.
- Air-monitoring report #6 then received a station-level monitor-grade review
  queue. New no-network script
  `air-monitoring/scripts/build-monitor-grade-station-review-queue.py` reads
  the one-signal review queue and the monitor-grade source-validation scan,
  then writes
  ...
  The queue assigns all 138 provenance-only station rows to review lanes: 66
  ...
  station-radius grade-assumption-ready rows at 0. The public route now
  renders a station-review wall with 5 stat cards, 3 lane cards, 7 country
  cards, 7 source-group cards, 12 sample station row cards, 5 evidence gates,
  and 3 download links. Chrome CDP QA at 1440x1100 and 390x1000 found no page
  or section horizontal overflow, no text overflow, and no page errors beyond
  ...
  `reporting-site/qa/showcase-air-grade-station-mobile-rows.png`. This is a
  station-level review queue, not station-level current grade certification
  and not station-radius population coverage.
- Air-monitoring report #6 then received a station method-evidence audit. New
  no-network script
  ...
  `reporting-site/qa/showcase-air-grade-method-mobile-rows.png`. This is exact
  station-row evidence triage, not station-level current grade certification
  and not station-radius population coverage.
- Air-monitoring report #6 then received a Uzbekistan station current/method
  scan. New networked script
  `air-monitoring/scripts/scan-uzbekistan-station-current-method-evidence.py`
  fetches the public Uzhydromet maps API, joins the 28 Uzbekistan exact-row
  instrument-hint rows by station ID, and writes
  ...
  keeps explicit current-status confirmed rows, complete monitor-grade rows,
  and station-radius-ready rows at 0. The public route now renders a
  Uzbekistan current/method panel with 6 stat cards, 3 age cards, 12 sample
  station row cards, 9 evidence gates, and 3 download links. Chrome headless QA
  at 1440x1100 and 390x1000 found no page or section horizontal overflow, no
...
- Air-monitoring report #6 then received a Uzbekistan method-policy source
  scan. New seeded source file
  `air-monitoring/source-inputs/uzbekistan-method-policy-source-seed.csv` and
  script `air-monitoring/scripts/scan-uzbekistan-method-policy-sources.py`
  scan 5 public official or technical sources and retrieve 4. The scan finds 4
  ...
  station-radius-ready rows at 0. This is source-policy context, not
  station-level closure. Chrome/Playwright QA at 1440x1100 and 390x1000
  confirmed the home report card, evidence note, and data-tab CSV/JSON are
  visible, with no console errors and no page/body horizontal overflow beyond
  existing React Router future-flag warnings. Screenshots:
...
- Air-monitoring report #6 then received a Uzbekistan station-specific source
  evidence scan. New seeded source file
  ...
  retrieve the official Uzhydromet map, 14 official regional station-table
  pages, 28 station-detail pages, and one official gov.uz ecology note. The
  scan parses 93 station rows, matches all 28 target Uzbekistan rows to
  official regional table rows, finds 28 station-detail URLs whose numeric path
  matches the internal target station ID, 26 detail measurements within 30
  days, 22 Horiba table-context rows, 6 `Updating data` rows, 2 official
  event-note station matches, and 1 negative sentinel detail PM2.5 value. It
  keeps current-status confirmed rows, complete monitor-grade classification
  rows, and station-radius-ready rows at 0. This is station-specific source and
  station-ID context, not current-status, grade, or catchment closure. Chrome
  CDP QA at 1440x1100 and 390x1000 confirmed the home report card,
  evidence-tab note link, and data-tab CSV/JSON links are visible, with no
  page-level horizontal overflow and no console/runtime errors. Screenshots:
...
- Air-monitoring report #6 then received a Uzbekistan endpoint-consistency
  check for the 3 exact blocker rows. New target seed
  `air-monitoring/source-inputs/uzbekistan-endpoint-consistency-targets.csv`
  and script
  `air-monitoring/scripts/scan-uzbekistan-endpoint-consistency.py` retrieve 16
  of 16 official source routes across the public Uzhydromet maps API,
  English/Russian/Uzbek station-detail pages, and English/Russian/Uzbek
  regional rows. The scan finds 9 of 9 language detail pages, 9 language
  regional rows, 3 cross-language detail agreement rows, 3 API/detail date
  mismatches, 2 API/detail PM2.5 mismatches, 3 region/detail status
  mismatches, 3 unresolved blocker rows, and 0 public endpoint-resolution,
  current-status confirmed, complete monitor-grade, or station-radius-ready
  rows. This is an endpoint QA wall, not a station-status or grade closure.
- Air-monitoring report #6 then received a BMKG API telemetry/status-field
  check. New source seed
  `air-monitoring/source-inputs/bmkg-api-parity-source-seed.csv` and script
  `air-monitoring/scripts/scan-bmkg-api-parity-status.py` follow the public
  BMKG app token flow, retrieve the PM2.5 list API, and retrieve 22 target
  station-detail API routes. The scan finds 24 list API station rows, 21
  target station files in the list API, 3 extra list API station files outside
  the target queue, 22 target detail API routes, 132 hourly PM2.5 observations,
  22 detail-coordinate rows, 21 list/detail coordinate matches, and 0
  station-status, inspection, calibration, certificate, grade, method,
  current-status confirmed, complete monitor-grade, or station-radius-ready
  rows. This is an API-field wall, not station-status or calibration closure.
- Air-monitoring report #6 then received a Georgia verification-policy wall.
  New source seed
  `air-monitoring/source-inputs/georgia-verification-policy-source-seed.csv`
  and script `air-monitoring/scripts/scan-georgia-verification-policy.py`
  retrieve 5 official `air.gov.ge` or MEPA policy/report/network/plan source
  routes and join them to the prior 24-month Georgia report/export ladder. The
  scan finds 1 live-data-not-verified policy source, 1 source saying verified
  data are available in reports, 1 report-generator source, 2 network or
  instrument-context sources, 1 validation/capture-rate context source, 24
  HTML months retaining `Not Verified Data`, 3 PDF export probes retaining the
  not-verified footer, and 0 verified report-closure, current-status,
  station-method, complete-grade, or station-radius-ready rows. This is a
  policy/report bridge, not station-status or verified-report closure.
- Air-monitoring report #6 then received a Georgia report-frequency matrix.
  New source seed
  `air-monitoring/source-inputs/georgia-report-frequency-matrix-source-seed.csv`
  and script `air-monitoring/scripts/scan-georgia-report-frequency-matrix.py`
  probe 24 official `air.gov.ge` daily, monthly, and annual HTML/XLSX/PDF
  report routes for the same 16 target station codes. The scan retrieves 12
  valid daily/monthly payloads, records 12 annual server-error probes, finds 8
  HTML/PDF payloads retaining not-verified labels, finds 4 XLSX exports with
  all target station sheets but 0 verification labels, and keeps verified
  report closure, current-status, station-method, complete-grade, and
  station-radius-ready rows at 0. This is a frequency-route falsifier, not a
  station-status or verified-report closure.
...
- PSDQ report #4 then received a source-repair official-coordinate evidence
  pass. New live public-source script
  `public-service-data-quality/scripts/explain-bgd-facility-source-repair-official-coordinates.py`
  retrieves the four public DGHS profile pages, parses the embedded official
  map coordinate, and compares it with the pinned OSM candidate coordinate.
  It writes
  ...
  The pass retrieves all 4 profiles, finds 4 exposed official coordinates,
  confirms all 4 match the inspection CSV, records 2 rows sharing one official
  profile coordinate, records 2 rows at least 10 kilometers from the named OSM
  candidate, records 1 row at least 50 kilometers from the named OSM candidate,
  and finds 0 explicit coordinate-source explanations. It closes or
  reclassifies 0 rows. This is public-source coordinate evidence, not human
  validation or source repair completion. Verification passed:
  official-coordinate script rerun, new script `py_compile`, program-script
...
- PSDQ report #4 then received a source-repair public-explanation evidence
  search. New live public-source script
  `public-service-data-quality/scripts/search-bgd-facility-source-repair-public-explanations.py`
  reads the four-row official-coordinate evidence CSV, joins cached DGHS public
  registry records, checks live DGHS profile tabs, and fetches linked official
  government health portals where available. It writes
  ...
  rows sharing one official profile coordinate; records 1 same-name
  cross-district DGHS registry sibling; and records 1 same-name other-district
  coordinate within 2 kilometers. The Netrakona Durgapur row is 747.0 meters
  from the separate Rajshahi Durgapur official record. It closes or reclassifies
  0 rows and does not mark any row human-validated. Verification passed:
  public-explanation script rerun, program-script `py_compile`, production site
  build, six deterministic gates plus `git diff --check`, review packet and zip
  rebuild, and agent-browser desktop/mobile QA at 1440x1100 and 390x900 with no
  page-level horizontal overflow and no page errors. Screenshots:
...
- PSDQ report #4 then received a source-repair correction-record follow-up.
  ...
  reads the public-explanation evidence CSV and targets the 2
  shared-coordinate Narayanganj rows plus the Durgapur same-name
  ...
  The pass checks and retrieves 20 public official sources, finds 0 public
  correction or coordinate-source records, confirms target DGHS dashboard codes
  for all 3 rows, confirms the linked Rajshahi Durgapur dashboard code for the
  Durgapur conflict, and closes or reclassifies 0 rows. Browser QA passed at
  1440x1100 and 390x900 with no page-level horizontal overflow and no page
  errors; console output only showed existing React DevTools / React Router
...
- PSDQ report #4 then received a no-contact source-repair clarification
  packet. New no-network script
  `public-service-data-quality/scripts/build-bgd-facility-source-repair-clarification-packet.py`
  reads the correction-record follow-up CSV and writes
  ...
  The packet creates 3 source-owner or human-review questions: 2
  ...
  Browser QA passed at 1440x1100 and 390x900 with 3 rendered cards, Durgapur
  and linked code `10000470` visible, no page-level horizontal overflow, and no
  page errors; console output only showed existing React Router development
  warnings. Screenshots:
...
- PSDQ report #4 then received a registry-vintage review. New no-network
  script
  `public-service-data-quality/scripts/build-bgd-facility-source-repair-registry-vintage-review.py`
  reads the clarification packet, public-explanation evidence CSV, and
  correction-record follow-up CSV. It writes
  ...
  The review covers the same 3 unresolved rows; all 3 have public DGHS profile
  update timestamps; those timestamps were 1 to 12 days old at
  public-explanation retrieval; 0 public correction or coordinate-source
  records were found; and 0 rows are allowed for closure, same-facility
  reclassification, or map-absence language. Browser QA passed at 1440x1100
  and 390x900 with 3 rendered cards, Durgapur and linked code `10000470`
  visible, the 1-12 day age range visible, no page-level horizontal overflow,
  and no page errors; console output showed only existing Vite / React Router
  development warnings. Screenshots:
...
## 2026-06-20 — Air-monitoring BMKG regional status/source gate
...
The generated summary records 22 BMKG target rows, 10/10 regional or public
source URLs retrieved, 1 official regional station-status source retrieved, 4
official regional-analysis context sources retrieved, 5 rows with exact
station-name or official site-variant external context, 3 rows with
regional-analysis context, 1 Banjarbaru row with official regional `ONLINE`
status and a recent timestamp, and 1 current-status confirmed row. The same
artifact keeps station-specific inspection-log, calibration-certificate/status,
complete monitor-grade, and station-radius-ready rows at 0. This is a useful
source-context expansion: current status is no longer zero for BMKG, but
complete grade still remains blocked.
...
and browser-checked desktop 1440x1100 plus mobile 390x1000. Browser QA found
6 stat cards, 3 decision lanes, 5 evidence rows, 10 gate cards, 10 source
cards, 3 working download links, no missing required text, no console errors,
no page errors, no request failures, and no page or section overflow.
Screenshots were refreshed at:
...
## 2026-06-20 — Air-monitoring BMKG dashboard current-status gate

Air-monitoring now adds
`air-monitoring/generated/air-monitoring-bmkg-dashboard-status-source-scan-summary.json`
to the public evidence stack for `/showcase/air-monitoring-observability`.
The new source pass adds
`air-monitoring/source-inputs/bmkg-dashboard-status-source-seed.csv` and
`air-monitoring/scripts/scan-bmkg-dashboard-status-sources.py`, generates
`air-monitoring-bmkg-dashboard-status-source-scan.csv`, and writes
`air-monitoring/bmkg-dashboard-status-source-scan.md`.

The generated summary records 22 BMKG target rows, 2/2 official BMKG
parent/dashboard source URLs retrieved, 26 dashboard locations parsed from the
public CEWS `dashboardData` object, all 22 target rows matched to dashboard
locations, 22 current dashboard timestamps, 21 current `ONLINE` rows, 1 current
`DELAYED` Pekanbaru row, 21 positive latest PM2.5 rows, and 2,640 target
time-series observations. The same artifact keeps station-specific inspection
logs, calibration-certificate/status, complete monitor-grade, and
station-radius-ready rows at 0. This moves the BMKG current-status evidence
wall, but it does not move grade or radius readiness.

Verification in this pass ran `py_compile`, reran the BMKG dashboard-status
source scan, synced public evidence and docs through the reporting-site build,
and browser-checked desktop 1440x1100 plus mobile 390x1000. Browser QA found
6 stat cards, 2 decision lanes, 22 status tiles, 10 gate cards, 2 source cards,
3 working download links, no missing required text, no console errors, no page
errors, no request failures, no overflowing children, and no page or section
overflow. The screenshot pass hides the sticky site header only during capture
so the saved QA images are clean. Screenshots were written at:
`reporting-site/qa/showcase-air-bmkg-dashboard-desktop-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-dashboard-desktop-status-wall-clean.png`,
`reporting-site/qa/showcase-air-bmkg-dashboard-desktop-sources-clean.png`,
`reporting-site/qa/showcase-air-bmkg-dashboard-desktop-gates-clean.png`,
`reporting-site/qa/showcase-air-bmkg-dashboard-mobile-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-dashboard-mobile-status-wall-clean.png`,
`reporting-site/qa/showcase-air-bmkg-dashboard-mobile-sources-clean.png`, and
`reporting-site/qa/showcase-air-bmkg-dashboard-mobile-gates-clean.png`.

## 2026-06-20 — Air-monitoring BMKG grade-basis source gate
...
`air-monitoring/scripts/scan-bmkg-grade-basis-sources.py`, generates
`air-monitoring-bmkg-grade-basis-source-scan.csv`, and writes
`air-monitoring/bmkg-grade-basis-source-scan.md`.

The generated summary records 22 BMKG target rows and 10/10 official BMKG
grade-basis sources retrieved. It finds 8 source-level method-basis sources, 7
...
The same artifact records 0 target rows with station-name context in those
sources, 0 station-specific inspection logs, 0 station-specific calibration
certificate/status rows, 0 current-status confirmations from this pass, 0
complete monitor-grade rows, and 0 station-radius-ready rows. This strengthens
the source-level method wall, but it does not move station certification or
radius readiness.

Verification in this pass ran `py_compile`, reran the BMKG grade-basis source
scan, synced public evidence/docs/references, built the reporting site, and
browser-checked desktop 1440x1100 plus mobile 390x1000. Browser QA found 6
stat cards, 3 source-family cards, 1 decision lane, 10 source cards, 11 gate
cards, 3 working download links, no missing required text, no console errors,
no page errors, no request failures, no bad HTTP responses, no overflowing
children, and no page or section overflow. Screenshots were written at:
`reporting-site/qa/showcase-air-bmkg-grade-basis-desktop-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-grade-basis-desktop-sources-clean.png`,
`reporting-site/qa/showcase-air-bmkg-grade-basis-desktop-gates-clean.png`,
`reporting-site/qa/showcase-air-bmkg-grade-basis-mobile-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-grade-basis-mobile-sources-clean.png`,
and
`reporting-site/qa/showcase-air-bmkg-grade-basis-mobile-gates-clean.png`.

## 2026-06-20 — Air-monitoring BMKG station public-context gate

Air-monitoring now adds
`air-monitoring/generated/air-monitoring-bmkg-station-public-context-source-scan-summary.json`
to the public evidence stack for `/showcase/air-monitoring-observability`.
The new source pass adds
`air-monitoring/source-inputs/bmkg-station-public-context-source-seed.csv` and
`air-monitoring/scripts/scan-bmkg-station-public-context-sources.py`, generates
`air-monitoring-bmkg-station-public-context-source-scan.csv`, and writes
`air-monitoring/bmkg-station-public-context-source-scan.md`.

The generated summary records 22 BMKG target rows and 9/9 public
station-unit, academic/journal, regulator, and deployment-context sources
retrieved. It finds 9 rows with public station or deployment context, 4
station-unit or exact-context rows, 6 city/deployment context rows, 9
method-context rows, 7 calibration-language context rows, and 8 inspection or
operating-context rows. The same artifact records 0 station-specific
inspection logs, 0 station-specific calibration certificate/status rows, 0
current-status confirmations from this pass, 0 complete monitor-grade rows,
and 0 station-radius-ready rows. This strengthens the reader-facing station
context wall, but station studies, city/deployment context, and historical
station-unit publications are still not station certificates.

Verification in this pass ran `py_compile`, reran the BMKG station
public-context source scan, synced public evidence/docs/references, built the
reporting site, and browser-checked desktop 1440x1100 plus mobile 390x1000.
Browser QA found 6 stat cards, 4 decision lanes, 9 matched row cards, 9 source
cards, 7 gate cards, 3 working download links, required text visible, no
console errors, no page errors, no request failures, no bad HTTP responses, no
overflowing children, and no page or section overflow. Screenshots were
written at:
`reporting-site/qa/showcase-air-bmkg-station-context-desktop-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-station-context-desktop-rows-clean.png`,
`reporting-site/qa/showcase-air-bmkg-station-context-desktop-sources-clean.png`,
`reporting-site/qa/showcase-air-bmkg-station-context-desktop-gates-clean.png`,
`reporting-site/qa/showcase-air-bmkg-station-context-mobile-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-station-context-mobile-rows-clean.png`,
`reporting-site/qa/showcase-air-bmkg-station-context-mobile-sources-clean.png`,
and
`reporting-site/qa/showcase-air-bmkg-station-context-mobile-gates-clean.png`.

## 2026-06-20 — Air-monitoring BMKG installation/audit source gate
...
`air-monitoring/scripts/scan-bmkg-installation-audit-sources.py`, generates
`air-monitoring-bmkg-installation-audit-source-scan.csv`, and writes
`air-monitoring/bmkg-installation-audit-source-scan.md`.

The generated summary records 22 BMKG target rows and 6/6 official BMKG
installation, audit/calibration, public-information, and operational-monitoring
sources retrieved. It finds 8 rows with installation or audit context, 1 exact
station audit/calibration context row, 7 PM2.5 installation/deployment context
rows, and 4 source-level operational or calibration routes. The same artifact
...
stronger official-source context wall, but installation, audit, NOC, PPID, and
AWS context are still not station certificates.

Verification in this pass ran `py_compile`, reran the BMKG installation/audit
source scan, synced public evidence/docs/references, built the reporting site,
and browser-checked desktop 1440x1100 plus mobile 390x1000. Browser QA found 6
stat cards, 3 decision lanes, 8 matched row cards, 6 source cards, 7 gate cards,
3 working download links, required text visible, no console errors, no page
errors, no request failures, no bad HTTP responses, no overflowing children, and
no page or section overflow. Screenshots were written at:
`reporting-site/qa/showcase-air-bmkg-install-audit-desktop-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-install-audit-desktop-rows-clean.png`,
`reporting-site/qa/showcase-air-bmkg-install-audit-desktop-sources-clean.png`,
`reporting-site/qa/showcase-air-bmkg-install-audit-desktop-gates-clean.png`,
`reporting-site/qa/showcase-air-bmkg-install-audit-mobile-section-clean.png`,
`reporting-site/qa/showcase-air-bmkg-install-audit-mobile-rows-clean.png`,
`reporting-site/qa/showcase-air-bmkg-install-audit-mobile-sources-clean.png`,
and
...
to the public evidence stack for `/showcase/air-monitoring-observability`.
The source pass adds
`air-monitoring/source-inputs/georgia-station-network-launch-source-seed.csv`
and `air-monitoring/scripts/scan-georgia-station-network-launch-sources.py`,
generates `air-monitoring-georgia-station-network-launch-source-scan.csv`, and
writes `air-monitoring/georgia-station-network-launch-source-scan.md`.

The generated summary records 16 Georgia target station-code rows and 8/8
official NEA current-network or station-launch pages retrieved. It finds 15
rows with official city or source context, 13 launch-context rows, 15
current-network city-context rows, and 15 PM2.5 or standard-equipment context
rows. The same artifact records 0 station-code rows in this source family, 0
verified-report closure rows, 0 current-status rows, 0 calibration-status rows,
0 complete monitor-grade rows, and 0 station-radius-ready rows. This is a
stronger station-owner context wall, but city-level launch/network pages are
still not station-code verification, calibration, certificate, status, or grade
evidence.

Verification in this pass ran `py_compile`, reran the Georgia NEA
network/launch source scan, synced public evidence/docs/references, built the
reporting site, and browser-checked desktop 1440x1100 plus mobile 390x1000.
Browser QA found 6 stat cards, 9 city cards, 3 decision lanes, 16 row cards, 8
source cards, 8 gate cards, 3 working download links, required text visible, no
page errors, no bad HTTP responses, no overflowing children, and no page or
section overflow. Screenshots were written at:
...
`reporting-site/qa/showcase-air-georgia-network-mobile-sources-clean.png`, and
