...
subtitle: A two-page evidence brief for connectivity monitoring across ADB developing member economies.
kind: brief
status: draft
...
Do not use national 4G coverage as a proxy for internet adoption. In the 34 ADB
developing member economies with exact-year ITU observations in 2024, reported
coverage exceeds use by a median **14.3 percentage points**; 31 differences are
positive.
...
The difference is not a person-level count and does not measure speed,
reliability, household affordability, skills, or impact.

## What else the evidence shows

- In a 28-economy balanced panel, the median gap narrows by 5.9 points from
  2018 to 2024, but widens in 12 economies.
- The national 5 GB price basket has a weak association with the 2024 gap
  (Spearman 0.16; n=32), so one price measure does not explain the pattern.
- The urban–rural use difference has a strong association in the ten matched
  cases (0.93), but the sample is too small for a regional mechanism claim.
- The headline is unchanged when the roster-coverage floor varies from 25% to
  75%.
...
3. Add household affordability, device, skill, and location data before
   assigning a barrier.
