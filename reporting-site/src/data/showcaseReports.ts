...
export const showcaseReportDepth: Record<number, ShowcaseReportDepth> = {
  ...
  6: {
    ...
    limitation: "Ground-monitor visibility is not pollution ranking, compliance assessment, or monitor siting adequacy; the reconciliation audit has candidate rows but zero validated joins, the exact-row method audit finds raw live-value issues, the Uzbekistan scans show API presence can carry stale reading dates and official tables can expose station-specific rows without internal station-ID/current-status closure, and complete monitor-grade classification plus catchment coverage remain at zero.",
  },
  7: {
...
export const showcaseReportQuality: Record<number, ShowcaseReportQuality> = {
  ...
  6: {
    ...
    nextUpgrade: "Search station-owner or regulator documentation that names internal Uzbekistan station IDs, explicit current operating status, or station-specific certification beyond the official regional table, then resolve the 38 Indonesia/Georgia PM2.5 portal/API rows without promoting any catchment denominator until grade evidence is complete.",
  },
  7: {
    readiness: "l3-candidate",
    readinessLabel: "L3 candidate",
