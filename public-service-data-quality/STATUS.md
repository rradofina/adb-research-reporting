...
- **2026-07-18:** Closed the current-issue figure dossier at six logical
  figures. Added a committed-data sensitivity range showing PHL at 14.5%–17.9%
  and BGD at 11.6%–11.8% across completed runs, both below the 30% screen, and
  a validation-wall infographic showing 40 targeted public-source rows, 39
  human/source-owner wall rows, and 0 AI-actionable closures. Both figures are
  generated by `scripts/build-figure-dossier.py`, documented in
  `figure-plan.md`, and embedded beside robustness and limitations in the
  canonical working paper. This completes the presentation move; it does not
  repair or validate a facility row.
...
**Stopping decision:** rotate. The current-issue paper, evidence packet, and
six-figure research spine are complete. Do not take another automated pass at
the row-validation wall unless a named public source plausibly changes a
claim-enabling count. Human/source-owner validation remains an owner-only
upgrade, so the factory proceeds to `remittance-resilience`.
