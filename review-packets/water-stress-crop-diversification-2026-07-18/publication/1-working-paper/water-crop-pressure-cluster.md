...
The distinction matters because the underlying literature supports hypotheses,
not the inherited formula. Cross-country evidence indicates that crop diversity
can stabilize national food production [@renard2019cropdiversity]. District
evidence from India finds that diversification can reduce losses from rainfall
deficits and heat stress, particularly under severe shocks
[@birthal2019cropdiversification]. Those studies evaluate production outcomes
over time. They do not imply that low cereal yield measures diversity or that a
national concentration index multiplied by water stress measures resilience.
...
The research question is therefore narrower than “where is water-crop pressure
highest?” It is: **does the inherited four-country set survive its stated
ranking rule and direct national measures of available-water stress and crop
concentration?**
...
The validation adds two public objects. First, WDI/AQUASTAT SDG 6.4.2 measures
freshwater withdrawal relative to available renewable freshwater resources
after environmental-flow requirements [@fao2017sdg642]. It has a latest 2022
observation for 30 roster economies. Second, the pipeline parses FAOSTAT Crops
and Livestock Products Area harvested rows for crop year 2024
[@fao2026faostatqcl]. After excluding aggregate item labels, it calculates crop
...
The sources are also not a balanced panel. Water stress is latest 2022, crop
mix is 2024, and the rural share reaches 2025. The analysis treats this as a
source-alignment diagnostic, not a common-year exposure estimate.

# Methodology

## Reconstructing the inherited screen
...
## Replacing the two named constructs
...
requirements. AQUASTAT cautions that renewable-water totals are not equivalent
to water that is economically or physically exploitable [@fao2024aquastat].
The measure improves the denominator but remains national and does not identify
depletion, seasonal scarcity, or transboundary allocation.

The crop replacement uses each crop item's share of total positive harvested
area. Concentration is summarized as a Herfindahl-Hirschman index:

\[
HHI_i = \sum_c s_{ic}^2,
\]

where \(s_{ic}\) is crop \(c\)'s harvested-area share in economy \(i\). A value
near one indicates that one crop occupies most reported harvested area. The
pipeline also calculates Shannon equitability and the top-one and top-three
shares, but HHI is used for the direct rank comparison.

## Validation tests

Five tests are committed.
...
4. **Association.** Calculate Spearman correlations among the old index,
   ...
   Confidence intervals use 5,000 deterministic bootstrap resamples.
5. **Sensitivity and ablation.** Vary the diagnostic water ceiling and the
   crop and rural exponents by ±50%, producing 27 specifications. Then remove
   components one at a time to see which dimension drives the ordering.

The source-upgraded diagnostic multiplies capped available-water stress, crop
HHI, and rural share. It is deliberately retained only as a diagnostic. A
composite can reveal driver dominance and membership sensitivity; it cannot
validate itself.

# Results

## The published set is not a persistent raw top four
...
The earlier statement converted “four economies appear in every top-five list”
into “four economies persistently occupy the top four.” Those are different
claims. The first is a set-intersection property; the second is contradicted by
the saved runs.

## The old water term cannot order its own leading economies
...
## Cereal yield did not measure crop diversification
...
## The two direct constructs do not align nationally
...
## The replacement diagnostic is dominated by water
...
This is why renaming the composite would not solve the problem. It would still
be a water ranking with a weakly associated crop term and a rural multiplier.
...
# Interpretation and related literature
...
descriptor, not resilience.

Likewise, SDG 6.4.2 is appropriate for national monitoring but does not assign
water pressure to crops. Aqueduct's multi-scale design and monthly scarcity
research show why basin, season, and demand location matter
[@wri2023aqueduct; @mekonnen2016waterscarcity]. The contribution here is not a
new water-risk ranking. It is evidence that those units cannot be skipped when
the claim combines water and agriculture.
...
The inherited ranking fails before the analysis reaches a policy question.
The published set is not the raw top four in five of seven saved runs. The old
water term saturates for all four leading internal-ratio economies, so cereal
yield and rural share determine their order. Direct available-water stress
retains only two published members; direct crop concentration retains none.
The crop-HHI leaders are then removed by missing water observations, and the
source-upgraded diagnostic remains almost entirely a water ranking.
...
records needed to audit every empirical statement.
